...
public class UpdatesListAdapter extends RecyclerView.Adapter<UpdatesListAdapter.ViewHolder> {
    ...
    public UpdatesListAdapter(UpdatesListActivity activity) {
        ...
        mActivity.getInstance().getTheme().resolveAttribute(android.R.attr.disabledAlpha, tv, true);
    ...
    private void handleActiveStatus(ViewHolder viewHolder, UpdateInfo update) {
        ...
        if (mUpdaterController.isDownloading(downloadId)) {
            canDelete = true;
            String downloaded = Formatter.formatShortFileSize(mActivity.getInstance(),
                    update.getFile().length());
            String total = Formatter.formatShortFileSize(mActivity.getInstance(), update.getFileSize());
            String percentage = NumberFormat.getPercentInstance().format(
                    update.getProgress() / 100.f);
            viewHolder.mPercentage.setText(percentage);
            long eta = update.getEta();
            if (eta > 0) {
                CharSequence etaString = StringGenerator.formatETA(mActivity.getInstance(), eta * 1000);
                viewHolder.mProgressText.setText(mActivity.getInstance().getString(
    ...
    private void handleNotActiveStatus(ViewHolder viewHolder, UpdateInfo update) {
        ...
        String fileSize = Formatter.formatShortFileSize(mActivity.getInstance(), update.getFileSize());
    ...
    public void onBindViewHolder(@NonNull final ViewHolder viewHolder, int i) {
        ...
        String buildDate = StringGenerator.getDateLocalizedUTC(mActivity.getInstance(),
                DateFormat.LONG, update.getTimestamp());
        String buildVersion = mActivity.getInstance().getString(R.string.list_build_version,
                update.getVersion());
    ...
    private void startDownloadWithWarning(final String downloadId) {
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(mActivity.getInstance());
        boolean warn = preferences.getBoolean(Constants.PREF_MOBILE_DATA_WARNING, true);
        if (Utils.isOnWifiOrEthernet(mActivity.getInstance()) || !warn) {
            mUpdaterController.startDownload(downloadId);
            return;
        }

        View checkboxView = LayoutInflater.from(mActivity.getInstance()).inflate(R.layout.checkbox_view, null);
        CheckBox checkbox = checkboxView.findViewById(R.id.checkbox);
        checkbox.setText(R.string.checkbox_mobile_data_warning);

        new AlertDialog.Builder(mActivity.getInstance())
                        ...
                        (dialog, which) -> {
                            if (checkbox.isChecked()) {
                                preferences.edit()
                                        ...
                                        .apply();
                                mActivity.getInstance().supportInvalidateOptionsMenu();
                            }
                            mUpdaterController.startDownload(downloadId);
                        })
    ...
    private void setButtonAction(Button button, Action action, final String downloadId,
        ...
        switch (action) {
            ...
            case REBOOT: {
                ...
                button.setEnabled(enabled);
                clickListener = enabled ? view -> {
                    PowerManager pm = mActivity.getInstance().getSystemService(PowerManager.class);
                    pm.reboot(null);
                } : null;
            }
            break;
    ...
    private AlertDialog.Builder getDeleteDialog(final String downloadId) {
        return new AlertDialog.Builder(mActivity.getInstance())
    ...
    private AlertDialog.Builder getInstallDialog(final String downloadId) {
        if (!isBatteryLevelOk()) {
            Resources resources = mActivity.getInstance().getResources();
        ...
        String buildInfoText = mActivity.getInstance().getString(R.string.list_build_version_date,
                update.getVersion(), buildDate);
        return new AlertDialog.Builder(mActivity.getInstance())
                .setTitle(R.string.apply_update_dialog_title)
                .setMessage(mActivity.getInstance().getString(resId, buildInfoText,
                        mActivity.getInstance().getString(android.R.string.ok)))
                .setPositiveButton(android.R.string.ok,
                        (dialog, which) -> {
                            Utils.triggerUpdate(mActivity.getInstance(), downloadId);
                            maybeShowInfoDialog();
                        })
                .setNegativeButton(android.R.string.cancel, null);
    }
    ...
    private AlertDialog.Builder getCancelInstallationDialog() {
        return new AlertDialog.Builder(mActivity.getInstance())
                ...
                .setPositiveButton(android.R.string.ok,
                        (dialog, which) -> {
                            Intent intent = new Intent(mActivity.getInstance(), UpdaterService.class);
                            intent.setAction(UpdaterService.ACTION_INSTALL_STOP);
                            mActivity.getInstance().startService(intent);
                        })
                .setNegativeButton(android.R.string.cancel, null);
    }
    ...
    private void startActionMode(final UpdateInfo update, final boolean canDelete, View anchor) {
        ...
        ContextThemeWrapper wrapper = new ContextThemeWrapper(mActivity.getInstance(),
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            ...
            if (itemId == R.id.menu_delete_action) {
                ...
                Utils.addToClipboard(mActivity.getInstance(),
                        mActivity.getInstance().getString(R.string.label_download_url),
                        update.getDownloadUrl(),
                        mActivity.getInstance().getString(R.string.toast_download_url_copied));
                return true;
    ...
    private void showInfoDialog() {
        String messageString = String.format(StringGenerator.getCurrentLocale(mActivity.getInstance()),
                mActivity.getInstance().getString(R.string.blocked_update_dialog_message),
                Utils.getUpgradeBlockedURL(mActivity.getInstance()));
        SpannableString message = new SpannableString(messageString);
        Linkify.addLinks(message, Linkify.WEB_URLS);
        if (infoDialog != null) {
            infoDialog.dismiss();
        }
        infoDialog = new AlertDialog.Builder(mActivity.getInstance())
    ...
    private boolean isBatteryLevelOk() {
        Intent intent = mActivity.getInstance().registerReceiver(null,
        ...
        int required = (plugged & BATTERY_PLUGGED_ANY) != 0 ?
                mActivity.getInstance().getResources().getInteger(R.integer.battery_ok_percentage_charging) :
                mActivity.getInstance().getResources().getInteger(R.integer.battery_ok_percentage_discharging);
